...
public class ServletScopeTest extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse response) throws ServletException, IOException {
		...
		//2.Browser가 살아있는동안 모든 JSP/Servlet에서 접근가능 
		HttpSession sess = req.getSession();
		//3.HttpServletRequest request : 요청이 살아있는 동안(요청이 바뀌지않는 동안)  JSP/Servlet에서 접근가능 
		
		app.setAttribute("appVar", "ServletContext는 server가 살아있는 동안 (started) 모든 JSP/Servlet에서 접근가능");
		sess.setAttribute("sessionVar", "HttpSession는 Browser가 살아있는동안 모든 JSP/Servlet에서 접근가능 ");
		...
		HttpServletRequest 속성은  해당 요청/응답에 대해서
